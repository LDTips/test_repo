        ...
        for i in range(3):
            try:
                if i == 0:
                    ...
                    print("diff_dict[{}][{}][0][{}]".format(file_type, key_split[0], key_split[1]))
                    diff_dict[file_type][key_split[0]][1][key_split[1]] = new_value
                elif i == 2:
...
def driver_universal_exp(key_split: list[str], diff_dict: dict, file_type: str, new_value: int | str) -> dict:
    message = "Could not assign key {}-{}-{}-{}. No match"
    if len(key_split) == 1:
        for i in range(2):
            try:
        ...
        for i in range(3):
            try:
                if i == 0:
                    ...
                    print("diff_dict[{}][{}][{}][{}]".format(file_type, key_split[0][:-1], key_split[0][-1], key_split[1]))
                    diff_dict[file_type][key_split[0][:-1]][int(key_split[0][-1])][key_split[1]] = new_value
                elif i == 2:
                    diff_dict[file_type][key_split[0]][0][key_split[1]][0] = new_value
                break
            except (KeyError, TypeError):
