...
def generate_start_script(daemons: {str or None: str}) -> str:
    ...
    try:  # f"{py_dir}\\scripts\\{file_name}.sh"
        with open(absolute_script_path, 'w+') as script:
            # Write out common lines:
            script.write(r"#!/bin/bash" + '\n')
            script.write(r"# AUTOGENERATED SCRIPT BY generate_start_script from vm commands" + '\n')
            ...
            for config_path, daemon in daemons.items():
                ...
                elif not config_path or len(config_path) == 0:  # Open5gs case - without config
                    script.write(f"/bin/open5gs-{daemon}d &\n")
                else:  # Open5gs case - with config
                    script.write(f"/bin/open5gs-{daemon}d -c {config_path} &\n")

            script.write("wait")  # Common part
    # TODO - Determine what other exceptions can happen, and handle them
    except PermissionError:
        logging.exception("Encountered permission error when trying to create a script!")
    else:
        return absolute_script_path  # Could be relative path, as paramiko file transfers accept relative paths too
